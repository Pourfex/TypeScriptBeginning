...
export class Game {
  ...
  private async getGameStatistics(): Promise<void> {
    const stats = await this.calculateStats([
      ...this.filterManager.getCurrentPeople(),
    ]);

    // Destructuring the statistics object
    const { totalPeople, demographics, ...otherStats } = stats;
  ...
  private calculateStats(people: Person[]): Promise<{
    ...
    return new Promise(resolve => {
      ...
      const demographics = people.reduce(
        (acc, { hasHat, hasGlasses, hasBeard }) => {
          return {
      ...
      const averageFeatures =
        featureCounts.reduce((sum: number, count: number) => sum + count, 0) /
